...
1. **"LLM 嵌入 + 树模型 + SHAP"可解释管线模板（M3/B2）**：DNABERT 提取序列语义、LightGBM 处理高维稀疏嵌入、SHAP 归因——直接可作为我们 gRNA 效率/脱靶预测模型的"预测+归因"一体化范式：用 SHAP 找出决定 E. coli gRNA 效率的关键位置/碱基，反哺 M1 特征工程与答辩素材。
